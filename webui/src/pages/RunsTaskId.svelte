...
<script lang="ts">
  ...
  import { runsStatus } from "../lib/api";
  ...
  async function fetchTaskStatus() {
    return runsStatus(taskId)
      .then((d) => (task = d))
      .catch((e) =>
        toast.error(`Error getting run status for runId: ${taskId}`, {
  ...
  onMount(() => {
    ...
    const pollInterval = setInterval(async () => {
      const hasActive = task && (task.status === "pending" || task.status === "running");
  ...
  <div class="card">
    ...
    <div class="flex flex-col gap-[0.4rem]">
      <div class="flex gap-4 text-[0.88rem]">
        <span class="text-muted min-w-24">Command</span><span class="text-ink">{task.command}</span>
      </div>
      {#if task.run_type}
        <div class="flex gap-4 text-[0.88rem]">
          <span class="text-muted min-w-24">Run type</span><span class="text-ink">{task.run_type}</span>
        </div>
      {/if}
      <div class="flex gap-4 text-[0.88rem]">
        <span class="text-muted min-w-24">Label</span><span class="text-ink">{task.label}</span>
      </div>
      <div class="flex gap-4 text-[0.88rem]">
        <span class="text-muted min-w-24">Created</span><span class="text-ink">{formatDate(task.created_at)}</span>
      </div>
      <div class="flex gap-4 text-[0.88rem]">
        <span class="text-muted min-w-24">Started</span><span class="text-ink">{formatDate(task.started_at)}</span>
      </div>
      <div class="flex gap-4 text-[0.88rem]">
        <span class="text-muted min-w-24">Finished</span><span class="text-ink">{formatDate(task.finished_at)}</span>
      </div>
      {#if task.duration}
        <div class="flex gap-4 text-[0.88rem]">
          <span class="text-muted min-w-24">Duration</span><span class="text-ink">{task.duration}</span>
        </div>
      {/if}
    </div>
    ...
    {#if task.result}
      <details class="card mt-4">
        <summary class="cursor-pointer text-[0.88rem] text-muted">Persisted result</summary>
    ...
    {#if task.benchmark_text}
      <details class="card mt-4">
        <summary class="cursor-pointer text-[0.88rem] text-muted">Benchmark summary</summary>
