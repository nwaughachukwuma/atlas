...
<div class="page">
  <h2>📊 System Stats</h2>
  <p class="desc">Overview of the vector store, task queue, and system health.</p>
...
<style>
  ...
  .stat-card h3 { font-size: 0.85rem; color: var(--text-muted); margin-bottom: 0.5rem; text-transform: uppercase; letter-spacing: 0.05em; }
  .big-num { font-size: 2.5rem; font-weight: 700; color: var(--primary); }
  .health-badge { font-size: 1.1rem; font-weight: 600; }
  .healthy { color: var(--success); }
  .details { margin-bottom: 1rem; }
  .details h3 { margin-bottom: 0.75rem; }
  .meta-grid { display: flex; flex-direction: column; gap: 0.5rem; }
  .meta-row { display: flex; gap: 1rem; font-size: 0.88rem; align-items: flex-start; }
  .key { color: var(--text-muted); min-width: 120px; }
  code { font-size: 0.8rem; background: var(--bg3); padding: 0.15em 0.4em; border-radius: 4px; word-break: break-all; }
</style>
